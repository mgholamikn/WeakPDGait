...
class Lifter_Multi_View(nn.Module):
    def __init__(self):
        super(Lifter_Multi_View, self).__init__()

        self.upscale = nn.Linear((num_joints*2+num_joints)*4, 1024)
        self.res_common = res_block()
        self.res_pose1 = res_block()
        self.res_pose2 = res_block()
        self.res_cam1 = res_block()
        self.res_cam2 = res_block()
        self.res_trans1 = res_block()
        self.res_trans2 = res_block()
        self.pose3d = nn.Linear(1024, num_joints*3)
        self.enc_rot = nn.Linear(1024, 12)
    ...
    def forward(self, p2d, conf):
        ...
        # camera path
        xt = nn.LeakyReLU()(self.res_trans1(x))
        xt = nn.LeakyReLU()(self.res_trans2(xt))
        xc = self.enc_trans(xt)

        return x_pose, xc.view(-1,4,3)
