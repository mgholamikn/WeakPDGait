...
for idx,(subj,subj_date) in enumerate(zip(subjects_All,subjects_All_date)):
    ...
    data_normal=np.load('/data/mohsen/mgholami/WeakPDGait/outputs_finetuned/Predictions_'+subj+'.npy')
...
for ii in range(len(subjects_PD)):
    ...
    label_model.fit(label_matrix_temp, seed=1)
    weights=label_model.get_weights()
    weights/=np.sum(weights)

    data_Y=np.sum(weights*label_matrix,axis=1)

    ## if two or more labeling functions agree on a lable we put as 1 otherwize as zero.
